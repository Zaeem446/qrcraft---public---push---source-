...
// QRFY corner dot (center) styles - shifted to match UI positions
// Position 1 = default, positions 2-16 shifted forward by 1
...
// QRFY requires ALL colors as objects with type, rotation, colorStops
function makeColorObject(hex: string, useGradient?: boolean, hex2?: string) {
  if (useGradient && hex2) {
    return {
      type: 'linear',
      rotation: 45,
  ...
  // Solid color - still needs object format
  return {
    type: 'solid',
    rotation: null,
...
export function mapDesignToStyle(design: Record<string, any>) {
  ...
  // Shape / pattern - use QRFY numeric style IDs
  style.shape = {
    style: getShapeStyle(design.dotsType),
    color: design.dotsColor || '#000000',
    backgroundColor: design.bgTransparent ? 'transparent' : (design.backgroundColor || '#FFFFFF'),
  };

  // Corners - use QRFY numeric style IDs
  style.corners = {
    squareStyle: getCornerSquareStyle(design.cornersSquareType),
    dotStyle: getCornerDotStyle(design.cornersDotType),
